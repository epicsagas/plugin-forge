...
- **`doctor`가 배열 등 비정형 JSON manifest에서 크래시하지 않음.** manifest 루프와 독립 마켓 판정이 JSON 객체가 아닌 파일을 만나면 FAIL/INFO로 처리한다.
...
### Removed
- **Slash commands dropped entirely.** The `commands/` directory, the four `plugin-forge-*.md` delegation stubs, and the `commands/README.md` scaffold are gone. `create` no longer generates a `commands/` directory or a `commands` manifest key, and doctor no longer runs the thin-delegate check (`COMMAND_BODY_MAX_LINES` removed). The skill is the only interface: every host reads `skills/plugin-forge/SKILL.md` and calls `forge.py` directly. Commands were a second spec that had to be edited in lockstep with the skill, and every host addition risked missing one.

### Changed
- **`skills/plugin-forge/SKILL.md` rewritten in English.** The skill is read by agents, not people, so its body is now English throughout. Korean trigger phrases stay in the frontmatter `description` because users invoke the skill in Korean and the matcher needs them. Human-facing Korean documentation lives in `docs/i18n/ko/README.md` and is unaffected.
- AGENTS.md and CONTRIBUTING.md updated to the skills-only interface rule.
...
### Changed (grok catalog, folded from a mis-headed section)
